...
`data_decidua`, we randomly select a subset of genes which we proceed with, as a
whole analysis would be out of the scope of this vignette.

```{r}
set.seed(28)
randgenes <- sample(rownames(data_blood), 2500, replace=FALSE)
...
```{r}
set.seed(24)
res <- wasserstein.sc(sce.blood, sce.decidua, method="TS",permnum=1000)
head(res, n=10L)
```
