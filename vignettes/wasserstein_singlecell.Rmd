...
Since the decidua and blood cells have already been preprocessed, we can run
the test now.

```{r}
wsres <- wasserstein.sc(sce.blood, sce.decidua, permnum = 10000, "TS")
head(wsres, n=2L)
```
...
This method might yield misleading results, if only a subset of genes is
given.
...
| p.ad.gpd      | p-value of the  Anderson-Darling test to check whether the GPD actually fits the data well (otherwise NA) \* |
